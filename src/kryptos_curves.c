...
static size_t g_std_curves_nr = sizeof(g_std_curves) / sizeof(g_std_curves[0]);

#define kryptos_new_kryptos_curve_ctx(c) {\
    (c) = (kryptos_curve_ctx *) kryptos_newseg(sizeof(kryptos_curve_ctx));\
    (c)->ec = NULL;\
    (c)->q  = NULL;\
    (c)->g  = NULL;\
}
...
kryptos_curve_ctx *kryptos_new_standard_curve(kryptos_curve_id_t id) {
    ...
    size_t c;
    int done = 0;
    kryptos_mp_value_t *a = NULL, *b = NULL, *p = NULL, *x = NULL, *y = NULL;

    for (c = 0; c < g_std_curves_nr; c++) {
        if (g_std_curves[c].id == id) {
            break;
        }
    }

    if (c == g_std_curves_nr) {
        goto kryptos_new_standard_curve_epilogue;
    }

    if ((p = kryptos_hex_value_as_mp(g_std_curves[c].p, strlen(g_std_curves[c].p))) == NULL) {
        goto kryptos_new_standard_curve_epilogue;
    }

    if ((a = kryptos_hex_value_as_mp(g_std_curves[c].a, strlen(g_std_curves[c].a))) == NULL) {
        goto kryptos_new_standard_curve_epilogue;
    }

    if ((b = kryptos_hex_value_as_mp(g_std_curves[c].b, strlen(g_std_curves[c].b))) == NULL) {
        goto kryptos_new_standard_curve_epilogue;
    }

    if ((x = kryptos_hex_value_as_mp(g_std_curves[c].x, strlen(g_std_curves[c].x))) == NULL) {
        goto kryptos_new_standard_curve_epilogue;
    }

    if ((y = kryptos_hex_value_as_mp(g_std_curves[c].y, strlen(g_std_curves[c].y))) == NULL) {
        goto kryptos_new_standard_curve_epilogue;
    }

    kryptos_new_kryptos_curve_ctx(curve);
    ...
    done = kryptos_ec_set_point(&curve->g, x, y);

    if (!done) {
        goto kryptos_new_standard_curve_epilogue;
    }

    done = ((curve->q = kryptos_hex_value_as_mp(g_std_curves[c].q, strlen(g_std_curves[c].q))) != NULL);

kryptos_new_standard_curve_epilogue:
...
void kryptos_del_curve_ctx(kryptos_curve_ctx *curve) {
    if (curve != NULL) {
        if (curve->ec != NULL) {
            kryptos_ec_del_curve(curve->ec);
        }

        if (curve->g != NULL) {
            kryptos_ec_del_point(curve->g);
        }
...
#undef kryptos_new_kryptos_curve_ctx
