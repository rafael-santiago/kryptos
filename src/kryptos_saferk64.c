...
static kryptos_u8_t kryptos_saferk64_e[] = { 0x01, 0x2d, 0xe2, 0x93, 0xbe, 0x45, 0x15, 0xae,
...
static kryptos_u8_t kryptos_saferk64_l[] = { 0x80, 0x00, 0xb0, 0x09, 0x60, 0xef, 0xb9, 0xfd,
...
typedef void (*kryptos_saferk64_block_processor)(kryptos_u8_t *block, struct kryptos_saferk64_subkeys sks);
...
static void kryptos_saferk64_block_encrypt(kryptos_u8_t *block, struct kryptos_saferk64_subkeys sks);

static void kryptos_saferk64_block_decrypt(kryptos_u8_t *block, struct kryptos_saferk64_subkeys sks);
...
static void kryptos_saferk64_ld_user_key(kryptos_u32_t key[2], const kryptos_u8_t *user_key, const size_t user_key_size) {
    ...
    kryptos_ld_user_key_byte(key[w], kp, kp_end, kryptos_saferk64_ld_user_key_epilogue);
    kryptos_ld_user_key_byte(key[w], kp, kp_end, kryptos_saferk64_ld_user_key_epilogue);
    kryptos_ld_user_key_byte(key[w], kp, kp_end, kryptos_saferk64_ld_user_key_epilogue);
    kryptos_ld_user_key_byte(key[w], kp, kp_end, kryptos_saferk64_ld_user_key_epilogue);
...
static void kryptos_saferk64_make_key(const kryptos_u8_t *key, const size_t key_size, struct kryptos_saferk64_subkeys *sks) {
    ...
    for (i = 0; i < i_nr; i++) {
        c[i].side[0] =                     kryptos_saferk64_e[kryptos_saferk64_e[(9 * (i + 2) + 1) % 257]];
        c[i].side[0] = c[i].side[0] << 8 | kryptos_saferk64_e[kryptos_saferk64_e[(9 * (i + 2) + 2) % 257]];
        c[i].side[0] = c[i].side[0] << 8 | kryptos_saferk64_e[kryptos_saferk64_e[(9 * (i + 2) + 3) % 257]];
        c[i].side[0] = c[i].side[0] << 8 | kryptos_saferk64_e[kryptos_saferk64_e[(9 * (i + 2) + 4) % 257]];
        c[i].side[1] = 		           kryptos_saferk64_e[kryptos_saferk64_e[(9 * (i + 2) + 5) % 257]];
        c[i].side[1] = c[i].side[1] << 8 | kryptos_saferk64_e[kryptos_saferk64_e[(9 * (i + 2) + 6) % 257]];
        c[i].side[1] = c[i].side[1] << 8 | kryptos_saferk64_e[kryptos_saferk64_e[(9 * (i + 2) + 7) % 257]];
        c[i].side[1] = c[i].side[1] << 8 | kryptos_saferk64_e[kryptos_saferk64_e[(9 * (i + 2) + 8) % 257]];
    }
...
static void kryptos_saferk64_block_encrypt(kryptos_u8_t *block, struct kryptos_saferk64_subkeys sks) {
    ...
    for (r = 0; r < sks.rounds; r++) {
        ...
        c0 = kryptos_saferk64_e[c0];
        c1 = kryptos_saferk64_l[c1];
        c2 = kryptos_saferk64_l[c2];
        c3 = kryptos_saferk64_e[c3];
        c4 = kryptos_saferk64_e[c4];
        c5 = kryptos_saferk64_l[c5];
        c6 = kryptos_saferk64_l[c6];
        c7 = kryptos_saferk64_e[c7];
...
static void kryptos_saferk64_block_decrypt(kryptos_u8_t *block, struct kryptos_saferk64_subkeys sks) {
    ...
    for (r = sks.rounds; r >= 1; r--) {
        ...
        // INFO(Rafael): Third step.
        c0 = kryptos_saferk64_l[c0];
        c1 = kryptos_saferk64_e[c1];
        c2 = kryptos_saferk64_e[c2];
        c3 = kryptos_saferk64_l[c3];
        c4 = kryptos_saferk64_l[c4];
        c5 = kryptos_saferk64_e[c5];
        c6 = kryptos_saferk64_e[c6];
        c7 = kryptos_saferk64_l[c7];
