...
void kryptos_gost_setup(kryptos_task_ctx *ktask,
    ...
    if (ktask->mode == kKryptosCTR && ktask->ctr != NULL) {
        ktask->iv[KRYPTOS_GOST_BLOCKSIZE - 4] = ((*ktask->ctr) >> 24) & 0xFF;
        ktask->iv[KRYPTOS_GOST_BLOCKSIZE - 3] = (((*ktask->ctr) & 0xFF0000) >> 16) & 0xFF;
        ktask->iv[KRYPTOS_GOST_BLOCKSIZE - 2] = (((*ktask->ctr) & 0xFF00) >> 8) & 0xFF;
...
static void kryptos_gost_load_user_key(const kryptos_u8_t *key, const size_t key_size, struct kryptos_gost_subkeys *sks) {
    ...
    kryptos_ld_user_key_prologue(sks->K, 8, key, key_size, kp, kp_end, w, b, return);
        kryptos_ld_user_key_byte(sks->K, 8, w, b, kp, kp_end, kryptos_gost_load_user_key_epilogue);
        kryptos_ld_user_key_byte(sks->K, 8, w, b, kp, kp_end, kryptos_gost_load_user_key_epilogue);
        kryptos_ld_user_key_byte(sks->K, 8, w, b, kp, kp_end, kryptos_gost_load_user_key_epilogue);
        kryptos_ld_user_key_byte(sks->K, 8, w, b, kp, kp_end, kryptos_gost_load_user_key_epilogue);
        kryptos_ld_user_key_byte(sks->K, 8, w, b, kp, kp_end, kryptos_gost_load_user_key_epilogue);
        ...
        kryptos_ld_user_key_byte(sks->K, 8, w, b, kp, kp_end, kryptos_gost_load_user_key_epilogue);
        kryptos_ld_user_key_byte(sks->K, 8, w, b, kp, kp_end, kryptos_gost_load_user_key_epilogue);
    kryptos_ld_user_key_epilogue(kryptos_gost_load_user_key_epilogue, sks->K, 8, w, b, kp, kp_end);
}
