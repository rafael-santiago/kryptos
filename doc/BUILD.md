...
- [Advanced build commands for contributors](#advanced-build-commands-for-contributors)
    ...
    - [Skipping the HOTP and TOTP sync tests](#skipping-the-hotp-and-totp-sync-tests)
...
### Skipping the HOTP and TOTP sync tests

Synchronization tests of HOTP and TOTP are time consuming in order to skip them you should use the following options:

```
Sherlock@221B:~/src/kryptos/src# hefesto --no-hotp-sync-tests --no-totp-sync-tests
```
...
| ``--no-poly1305-tests``                  | Does not include Poly1305 tests in the test binary. Speeds up the build.     |
| ``--no-siphash-tests``                   | Does not include SipHash tests in the test binary. Speeds up the build.      |
| ``--no-hotp-sync-tests``                 | Skips HOTP sync tests. Speeds up the tests execution.                        |
| ``--no-totp-sync-tests``                 | Skips TOTP sync tests. Speeds up the tests execution.                        |
